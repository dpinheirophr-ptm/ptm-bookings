  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#00000066", zIndex:1000,
      ...
      <div style={{ background:"#ffffff", border:"1px solid #bbf7d0",
        borderRadius:"12px", padding:"20px", width:"100%", maxWidth:"500px",
        ...
        <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"16px" }}>
          <h2 style={{ color:"#166534", fontFamily:"monospace", fontSize:"15px", margin:0 }}>
            {job && job.id ? "Editar Job" : "Novo Job"}
          </h2>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px" }}>
          ...
          <div>
            <label style={lbl}>Horario on site</label>
          ...
          <div>
            ...
            <select style={inp} value={form.teamLeader} onChange={e=>set("teamLeader",e.target.value)}>
              <option value="">Selecionar...</option>
              {TL_LIST.map(n=><option key={n}>{n}</option>)}
            </select>
          </div>
          <div>
          ...
          <div style={{ gridColumn:"1/-1" }}>
              ...
              {TC_LIST.map(name => {
                ...
                return (
                  <div key={name} onClick={()=>toggleWorker(name)} style={{
                    ...
                    cursor:"pointer", background: selected ? "#166534" : "#f0fdf4",
                    color: selected ? "#ffffff" : "#166534",
                    border: "1px solid " + (selected ? "#166534" : "#bbf7d0"),
                    userSelect:"none"
                  }}>{name}</div>
                );
              })}
            ...
            <div style={{ color:"#64748b", fontSize:"11px", marginTop:"6px" }}>
              {(form.workers||[]).length} TCs selecionados
            </div>
          </div>
          <div style={{ gridColumn:"1/-1" }}>
            ...
            <textarea style={{ ...inp, minHeight:"56px", resize:"vertical" }}
              ...
              placeholder="Ex: $120 travel paid..." />
          </div>
        </div>
        <div style={{ display:"flex", gap:"10px", marginTop:"16px" }}>
          ...
          <button onClick={()=>onSave(form)} style={{ flex:2,
            ...
            border:"none", color:"#ffffff", borderRadius:"6px", padding:"10px",
...
function JobCard({ job, onEdit, onDelete, onToggle }) {
  ...
  function Checkbox(props) {
    return (
      <div onClick={()=>onToggle(job.id, props.field)} style={{ display:"flex", alignItems:"center", gap:"6px", cursor:"pointer" }}>
        <div style={{ width:"18px", height:"18px", borderRadius:"4px",
          background: job[props.field] ? props.color : "#f1f5f9",
          border: "2px solid " + (job[props.field] ? props.color : "#cbd5e1"),
          display:"flex", alignItems:"center", justifyContent:"center" }}>
          {job[props.field] && <span style={{ color:"#fff", fontSize:"12px", fontWeight:"bold" }}>v</span>}
        </div>
        <span style={{ color: job[props.field] ? props.color : "#94a3b8", fontSize:"12px", fontWeight:"500" }}>{props.label}</span>
      </div>
    );
  }
  ...
  return (
    <div style={{ background:"#ffffff", border:"1px solid " + sc.border + "44",
      ...
      padding:"14px 16px", marginBottom:"10px", boxShadow:"0 1px 4px rgba(0,0,0,0.06)" }}>
      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"8px", flexWrap:"wrap", gap:"6px" }}>
        ...
        <div style={{ display:"flex", gap:"6px", alignItems:"center" }}>
          ...
          <button onClick={()=>onDelete(job.id)} style={{ background:"#fff0f0", border:"1px solid #fecaca",
            color:"#ef4444", borderRadius:"4px", padding:"3px 8px", fontSize:"11px", cursor:"pointer" }}>X</button>
        </div>
      </div>
      <div style={{ fontSize:"12px", marginBottom:"6px" }}>
        <span style={{ color:"#3b82f6", fontWeight:"600" }}>Horario: {job.time}</span>
        <span style={{ color:"#cbd5e1", margin:"0 6px" }}>|</span>
        <a href={mapsUrl} target="_blank" rel="noreferrer" style={{ color:"#166534", fontSize:"12px" }}>
          Endereco: {job.address}
        </a>
        {job.workOrderRef && <span style={{ color:"#94a3b8", fontSize:"11px", marginLeft:"8px" }}>({job.workOrderRef})</span>}
      </div>
      {utes.length > 0 && (
        <div style={{ fontSize:"12px", marginBottom:"6px" }}>
          {utes.map((u,i) => (
      ...
      {job.notes && (
        <div style={{ color:"#64748b", fontSize:"11px", fontStyle:"italic", marginBottom:"8px",
          borderLeft:"3px solid #bbf7d0", paddingLeft:"8px", background:"#f0fdf4",
    ...
    <div style={{ minHeight:"100vh", background:"#f0fdf4", fontFamily:"Inter,sans-serif", color:"#1a2e1a" }}>
      ...
      <div style={{ padding:"14px", maxWidth:"700px", margin:"0 auto" }}>
        {loading ? (
          ...
          <>
            <div style={{ color:"#64748b", fontSize:"11px", fontFamily:"monospace", marginBottom:"10px" }}>
